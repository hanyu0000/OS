...
//信号量的down操作，即-1,传入参数是指向要操作的信号量指针。线程申请信号量的时候用此函数
void sema_down(struct semaphore* psema){
    enum intr_status old_status = intr_disable();//对于信号量操作必须关中断

    //一个自旋锁，来不断判断是否信号量已经被分配出去
    while(psema->value == 0){//若value为0,表示已经被别人持有
        ...
        //为什么不用判断是否在阻塞队列中呢？因为线程被阻塞后，会加入阻塞队列，除非被唤醒，否则不会
...
//获取锁的函数，传入参数是指向锁的指针
void lock_apquire(struct lock* plock){
    //排除线程自己已经拿到锁，但是没有释放就重新申请的情况
    if(plock->holder != running_thread()){
        sema_down(&plock->semaphore);//对信号量进行down操作
        plock->holder = running_thread();
        ASSERT(plock->holder_repeat_nr == 0);
        plock->holder_repeat_nr = 1;//申请锁
    }
    else
...
void lock_release(struct lock* plock){
    ...
    ASSERT(plock->holder_repeat_nr == 1);//判断现在lock的重复持有数为1，才能释放
